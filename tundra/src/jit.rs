...
use cranelift_codegen::ir::{types, AbiParam, Signature, types::I64, MemFlags};
...
use crate::vm::vm_helpers::{
    tundra_array_get, tundra_array_set, tundra_call_raw, tundra_get_global, tundra_invoke, tundra_new_array, tundra_pow, tundra_pow_i64, tundra_pow_i64_mod, tundra_print, tundra_set_global, tundra_store_register           
};
use crate::bytecode::{opcode::OpCode, value::{ValueType, FunctionObject}};
...
impl JitContext {
    pub fn new() -> Self {
        ...
        sig.params.push(AbiParam::new(I64));
        sig.returns.push(AbiParam::new(I64));
        let invoke_id = module.declare_function("tundra_invoke",     Linkage::Import, &sig).unwrap();

        let mut sig = Signature::new(module.isa().default_call_conv());
        sig.params.push(AbiParam::new(ptr_ty));
        sig.params.push(AbiParam::new(I64));
        sig.returns.push(AbiParam::new(ptr_ty));
        let new_array_id = module.declare_function("tundra_new_array", Linkage::Import, &sig).unwrap();
        ...
        sig.params.push(AbiParam::new(I64));
        sig.returns.push(AbiParam::new(I64));
        let array_get_id = module.declare_function("tundra_array_get", Linkage::Import, &sig).unwrap();
        ...
        sig.params.push(AbiParam::new(I64));
        sig.params.push(AbiParam::new(I64));
        let array_set_id = module.declare_function("tundra_array_set", Linkage::Import, &sig).unwrap();

        let mut sig = Signature::new(module.isa().default_call_conv());
        sig.params.push(AbiParam::new(ptr_ty));
        sig.params.push(AbiParam::new(I64));
        let print_id = module.declare_function("tundra_print", Linkage::Import, &sig).unwrap();
    ...
    let get_global_id = module
        ...
        sig.params.push(AbiParam::new(I64));
        sig.params.push(AbiParam::new(I64));
        let set_global_id = module.declare_function("tundra_set_global",
 ...
 let store_reg_id = module
     ...
     pub fn compile_function(&mut self, fobj: &mut FunctionObject) {
        ...
        let pow_i64_ref = self.module.declare_func_in_func(self.pow_i64_id, &mut ctx.func);
        let pow_mod_ref = self.module.declare_func_in_func(self.pow_mod_id, &mut ctx.func);
        let pow_ref     = self.module.declare_func_in_func(self.pow_id,     &mut ctx.func);
        let invoke_ref    = self.module.declare_func_in_func(self.invoke_id,    &mut ctx.func);
        let call_raw_ref  = self.module.declare_func_in_func(self.call_raw_id, &mut ctx.func);
        let new_array_ref = self.module.declare_func_in_func(self.new_array_id, &mut ctx.func);
        let array_get_ref = self.module.declare_func_in_func(self.array_get_id, &mut ctx.func);
        let array_set_ref = self.module.declare_func_in_func(self.array_set_id, &mut ctx.func);
        let print_ref     = self.module.declare_func_in_func(self.print_id,     &mut ctx.func);
        let get_global_ref = self.module.declare_func_in_func(self.get_global_id, &mut ctx.func);
        let set_global_ref = self.module.declare_func_in_func(self.set_global_id, &mut ctx.func);
         let store_reg_ref = self.module.declare_func_in_func(self.store_reg_id, &mut ctx.func);
        ...
        for (pc, op) in code.iter().enumerate() {
            match op {
                OpCode::JumpIfFalse(_, offset) => {
                    let fall = pc + 1;
                    let tgt  = pc + 1 + offset;
                    if fall < code_len { preds[fall].push(pc) }
                    if tgt  < code_len { preds[tgt] .push(pc) }
                }
                OpCode::Jump(offset) => {
                    let tgt = pc + 1 + offset;
                    if tgt < code_len { preds[tgt].push(pc) }
...
OpCode::JumpIfFalse(r, offset) => {
    ...
    let target  = if tgt_idx == code_len { exit_block } else { blocks[tgt_idx] };
...
OpCode::SetLocal(src, slot) => {
    ...
    let call    = fb.ins().call(call_raw_ref,
                                &[vm_ptr, base, abs, raw_arg]);
    ...
    fb.ins().call(array_set_ref, &[vm_ptr, arr_val, idx_val, val]);
}
                OpCode::Print(r) => {
...
OpCode::None(d) => {
   ...
   OpCode::GetGlobal(dest, ref name) => {
            ...
            let call = fb.ins().call(get_global_ref, &[vm_ptr, ptr_const, len_const]);
         ...
         OpCode::SetGlobal(src, ref name) => {
            ...
            fb.ins().call(set_global_ref, &[vm_ptr, ptr_const, len_const, val]);
         }
...
lazy_static! {
     ...
     pub static ref GLOBAL_NAMES: std::sync::Mutex<
