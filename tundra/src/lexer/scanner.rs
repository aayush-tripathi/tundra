...
impl Scanner {
        ...
        while !self.is_at_end() {
            ...
            if at_line_start {
                ...
                if saw_space && saw_tab {
                    ...
                    if count > last {
                        indents.push(count);
                        tokens.push(Token::new(TokenType::Indent, String::new(), self.current_line, self.current_line));
                    } else {
    ...
    '|' => {
        if self.match_char('=') {
            ...
            '!' => if self.match_char('=') { TokenType::BangEqual } else { TokenType::Bang },
            '=' => if self.match_char('=') { TokenType::EqualEqual } else { TokenType::Equal },
            '<' => if self.match_char('=') { TokenType::LessEqual } else { TokenType::Less },
            '>' => if self.match_char('=') { TokenType::GreaterEqual } else { TokenType::Greater },
    ...
    fn char_literal(&mut self) -> Token {
        ...
        if self.is_at_end() || self.peek() != '\'' {
            return Token::new(TokenType::Error, "Unterminated char literal".into(), self.start_line, self.current_line);
        }
        self.advance();
    ...
    fn identifier(&mut self) -> Token {
        ...
        let tok_type = self.keyword_trie.get_token_type(&text).unwrap_or(TokenType::Identifier);
