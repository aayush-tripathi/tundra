...
impl Value {
    ...
    pub fn native(func: fn(&[Value]) -> Value, arity: usize) -> Self {
        Value {
            value: ValueType::NativeFunction(func, arity),
        }
    }
    ...
    pub fn array(v: Vec<Value>) -> Self {
        Value {
            value: ValueType::Array(Rc::new(RefCell::new(v))),
        }
    }
    ///Convert to Bool
    ...
    pub fn as_i64(&self) -> i64 {
        match &self.value {
            ...
            ValueType::Function(rc) => {
                let p = Rc::as_ptr(rc) as u64;
                (p | TAG_PTR) as i64
            }
            ValueType::NativeFunction(f, _) => {
                let p = *f as *const () as u64;
                (p | TAG_PTR) as i64
            }
        }
    }
    ...
    pub unsafe fn from_i64(raw: i64) -> Self {
        ...
        match bits & TAG_BITS {
            ...
            TAG_PTR => Self {
                value: ValueType::from_heap_ptr((bits & !TAG_BITS) as *const ()),
            },
